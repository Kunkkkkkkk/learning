...
但是每一次都写这么多，所以我们可以把他们封装到工具类里。用泛型就行

#### redis工具封装

里面没难的，要思考的就是怎么传mybatis的方法进来。
...
public class CacheUtils {
    ...
    public  void setRedisWithTTL(String key,Object o, long ttl, TimeUnit timeUnit){
        String json = JSONUtil.toJsonStr(o);
        stringRedisTemplate.opsForValue().set(key,json,ttl,timeUnit);
    }
    public  void setRedisWithLogicExpire(String key,Object o ,long ttl, TimeUnit timeUnit) {
        RedisData redisData = new RedisData();
        redisData.setData(o);
        redisData.setExpireTime(LocalDateTime.now().plusSeconds(timeUnit.toSeconds(ttl)));
        stringRedisTemplate.opsForValue().set(key, JSONUtil.toJsonStr(redisData));
    }
    public <R,Tid> R getWithCacheThroughByNull(String prefix,  Tid id, Class<R> clazz, Function<Tid,R> function,Long ttl, TimeUnit timeUnit){
        String key =prefix + id.toString();
        String s = stringRedisTemplate.opsForValue().get(key);
        if(StrUtil.isNotBlank(s)){//区别于isNotEmpty
            return JSONUtil.toBean(s, clazz); //
        }
        if(s != null){
            return null;
        }
        //难点是怎么使用mybatisplus里的方法来访问数据库
        //可以传一个方法来
        R result=function.apply(id);
        //如果空的则存入空值
        if(result != null){
            setRedisWithTTL(key,result,ttl,timeUnit);
            return result;
        }else {
            stringRedisTemplate.opsForValue().set(prefix+id.toString(),"",30,TimeUnit.MINUTES);
            return null;
        }
    }

    public <R,Tid> R getWithCacheBrokenByLogicExpire(String prefix, Tid id, Function<Tid,R> function,Long ttl, TimeUnit timeUnit){
        //我们这里使用String类型的redis
        String key=prefix+id.toString();
        String s = stringRedisTemplate.opsForValue().get(key);
        RedisData redisData = JSONUtil.toBean(s,RedisData.class);
...
//TODO 就算获得了还要再次获取缓存检测是否过期，双重检查 ，原因我会写在文档并举例子
            ...
            RedisData redisData1 = JSONUtil.toBean(s1,RedisData.class);
            if (LocalDateTime.now().isBefore(redisData1.getExpireTime())) {
                return (R) redisData1.getData(); //返回旧的
            }
            /*Thread thread = new Thread(new Runnable() {
            ...
            threadpool.submit(()->{
                //查询数据库重新插入缓存
                R result = function.apply(id);
                setRedisWithLogicExpire(key,result,ttl,timeUnit);
                unlock(prefix+id.toString());
            });


            return (R) redisData.getData();
        } else return (R) redisData.getData();

    }
    private boolean lock(String key) {
        //TODO setnx是这个开始 空就可以拿到锁
