...
`redis-cli -a 密码 shutdown`可以关闭，前提是**避免重复查询数据库**：
...
- 当线程 A 查询数据库并更新了缓存后，缓存中已经有了最新的数据。如果其他线程获取到锁时，不进行缓存检查直接查询数据库并更新缓存，可能会导致数据的不一致性问题（特别是在并发较高或缓存失效较短的情况下）。

**提升性能**：

- 通过在锁内再做一次缓存检查，可以避免重复的数据库查询操作。只要线程 A 已经成功更新了缓存，线程 B 就不需要再查询数据库，而是直接使用缓存中的新数据。这样能显著提高系统的性能和吞吐量。

在conf文件里面protected-mode字段为no
