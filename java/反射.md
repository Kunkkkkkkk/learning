...
1.spring的IOC和DI，spring容器扫描配置或者注解，通过反射得到java对象，然后自动装配依赖。

2.AOP里面的动态代理，就要依赖反射拿到调用的方法和与方法有关的其他信息，而且Handler的invoke方法里面传了代理类、方法的Method类、参数。这里面的的Method类也是反射拿到的，而且执行这个也是使用了反射
